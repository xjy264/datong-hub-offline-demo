...
test('new station marker uses the slightly smaller round button size', () => {
  assert.equal(DEFAULT_MARKER_SIZE, 2.8)
})
...
test('marker css vars renders legacy 4.4px stations at the smaller 2.8px size', () => {
  assert.equal(markerCssVars({ x: 10, y: 20, size: 4.4 })['--size'], '2.8px')
})

test('marker css vars renders previous default sizes at the smaller 2.8px size', () => {
  assert.equal(markerCssVars({ x: 10, y: 20, size: 4 })['--size'], '2.8px')
  assert.equal(markerCssVars({ x: 10, y: 20, size: 3.6 })['--size'], '2.8px')
})
