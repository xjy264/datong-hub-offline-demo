...
export const useMapStore = defineStore('map', {
  ...
  actions: {
    ...
    async createInterval(mapId: string, body: { markerAId: string | null; markerBId: string | null; baseStations: string[]; x: number; y: number; length: number; angle: number }) {
    ...
    async updateProfile(stationId: string, body: { name: string; notes: string; mileage: string; workshopId: number | null; color?: 'red' | 'blue' }) {
    ...
    async createWorkshop(name: string) {
      const workshop = await apiPost<Workshop>('/workshops', { name })
      if (this.currentMap) await this.loadMap(this.currentMap.id)
      else await this.load(true)
      return workshop
    },
